...
def mock_google_agent():
    ...
    with respx.mock(assert_all_called=False, assert_all_mocked=False) as respx_mock:
        ...
        # Sequence: start_interview -> get_phases -> get_context (per phase)
        #           -> evaluate_phase (per turn)
        #
        # Phase 1: start_interview, get_phases, get_context,
        #          evaluate (continue, continue, next_phase)
